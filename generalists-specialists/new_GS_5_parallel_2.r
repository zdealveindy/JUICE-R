...
GS.function <- function (input.matrix, reps, psample, version = "multiplicative", beals.file, parallel = parallel, no.cores = no.cores, remove.out = remove.out, par.library = 'doSNOW') # the function calculating theta value of species specialization
{
...
{
require (foreach)
if (par.library == 'doSMP') {require (doSMP); workers <- startWorkers(no.cores, FORCE = F); registerDoSMP(workers)} else
   {require (doSNOW); workers <- makeCluster(no.cores, type = "SOCK"); registerDoSNOW(workers)}
...
#tkdestroy (base)
write.csv2 (file = "theta_out.csv", theta.out)
if (par.library == 'doSNOW') stopCluster (workers) else if (par.library == 'doSMP') stopWorkers (workers)
}
...
tkwm.title(base, "Calculation of generalists-specialists metric")
  ...
  tkpack (tklabel (frame.e.1, text = 'Choose the file with species matrix containing \nbeals smoothed data transformed into presences/absences '), tkbutton (frame.e.1, text = 'Choose', command = function () assign ('beals.file', choose.files (), env = .GlobalEnv)), side = 'left', anchor = 'w')
 tkpack (tklabel (frame.e, text = 'Beals smoothing (included in method of Botta-Dukat 2011)'), frame.e.1, anchor = 'w')

  
  parallel.label <- tklabel (frame.f, text = 'Parallel calculation (enable if you have more than one core)')
  ...
  parallel.checkbutton <- tkcheckbutton (frame.f, text = 'use parallel computing', variable = parallel)
  
  tkpack (tklabel (frame.g, text = 'Outlier analysis', anchor = 'w'), tkcheckbutton (frame.g, text = 'remove outlier samples (with very different species composition)', variable = remove.out), anchor = 'w')
  ...
  tkpack (button1)
  tkpack (parallel.no.cores.label, parallel.no.cores.entry)
  tkpack (parallel.label, parallel.checkbutton, frame.f.1)
  ...
  tkpack (frame.f, ipady = 10, ipadx = 10, padx = 10, pady = 10, anchor = "w")
  tkpack (frame.g, ipady = 10, ipadx = 10, padx = 10, pady = 10, anchor = "w")
