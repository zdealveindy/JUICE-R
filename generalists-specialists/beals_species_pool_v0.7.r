...
###
### apply.out - should be the outlier analysis for setting up the threshold applied? (default is TRUE, following the recommendation of Zoltan Botta-Dukat 2012 JVS paper)
###
### incl.sp.pool - should be species, occurring in the releve, be directly included also in the species pool of given releve (i.e. without considering their beals probabilities)?
###                Default = FALSE (i.e. it may happen that some species actually present in the releve will not occur in its species pool)
###
...
beals.spec.pool <- function (x, grouping, species.data = NULL, include = FALSE, cooc.matrix = 'background', write.to.txt = FALSE, verbal = T, apply.out = T, incl.sp.pool = F) 
{
	require (tcltk)
...
# which is recommended by Herben & Munzbergova
	...
	if (apply.out) beals.thresh <- apply (x.b*background, 2, FUN = function (y) outlier.fun(y)) else beals.thresh <- apply (x.b*background, 2, FUN = function (y) min(y[y>0]))
	...
	if (incl.sp.pool) spec.pool <- spec.pool | (target > 0)
	if (verbal) close (win.pb2)
...
beals.spec.pool.2 <- function (x, species.data = NULL, include = FALSE, cooc.matrix = 'background', write.to.txt = FALSE, verbal = T, apply.out = T, incl.sp.pool = F, parallel = F, no.cores = 4)
{
...
if (!parallel)
{
  ...
  for (i in seq (nrow (x)))
	 ...
	 grouping[i] <- 1
	 spec.pool[i,] <- beals.spec.pool (x, grouping = grouping, species.data = NULL, include = include, cooc.matrix = cooc.matrix, write.to.txt = FALSE, verbal = FALSE, apply.out = apply.out, incl.sp.pool = incl.sp.pool)
	}
}
...
beals.fun <- function (i, sp, include, cooc.matrix, apply.out)
  {
	grouping <- rep (2, nrow (sp))
	grouping[i] <- 1
	beals.spec.pool (sp, grouping = grouping, species.data = NULL, include = include, cooc.matrix = cooc.matrix, write.to.txt = FALSE, verbal = FALSE, apply.out = apply.out, incl.sp.pool = incl.sp.pool)
   }
#clusterExport (cl, list('x', 'include', 'cooc.matrix', 'beals.spec.pool'))
#clusterExport (cl, 'beals.fun')
clusterExport (cl, 'beals.spec.pool')
spec.pool <- parSapply (cl, seq (nrow (x)), FUN = beals.fun, sp = sp, include = include, cooc.matrix = cooc.matrix, apply.out = apply.out)
...
calculate.function <- function (parallel, no.cores, apply.out, incl.sp.pool)
{
if (length (unique (grouping)) == 1)
	sp.p <- beals.spec.pool.2 (x = x, species.data = species.data, include = T, cooc.matrix = 'background', write.to.txt = TRUE, parallel = parallel, no.core = no.cores, apply.out = apply.out, incl.sp.pool = incl.sp.pool)
if (length (unique (grouping)) == 2)
	sp.p <- beals.spec.pool (x = x, grouping = grouping, species.data = species.data, include = T, cooc.matrix = 'background', write.to.txt = TRUE, apply.out = apply.out, incl.sp.pool = incl.sp.pool)
...
#setwd("C:/Program Files/R/R-2.15.1/bin/x64/")
...
no.cores <- tclVar (2)
apply.out.anal <- tclVar (1)
include.species.sppool <- tclVar (0)
...
tkpack (tklabel (frame.d, text = 'Options:'))
tkpack (tkcheckbutton (frame.d, text = 'apply outlier analysis for setting species threshold', variable = apply.out.anal))
tkpack (tkcheckbutton (frame.d, text = 'include species in plot directly to species pool of that plot', variable = include.species.sppool))
...
tkpack (tkbutton (frame.b, text = 'Calculate', command = function () calculate.function (parallel = as.logical (as.numeric (tclvalue (parallel.comp))), no.cores = as.numeric (tclvalue (no.cores)), apply.out = as.logical (as.numeric (tclvalue (apply.out.anal))), incl.sp.pool = as.logical (as.numeric (tclvalue (include.species.sppool))) )), side = 'right')



tkpack (frame.a, side = "top", ipady = 10, ipadx = 10, padx = 10, pady = 10, anchor = "w", fill = 'both')
tkpack (frame.b, side = 'right', ipady = 2, ipadx = 2, padx = 10, pady = 10, anchor = 'w')
tkpack (frame.c,  ipady = 10, ipadx = 10, padx = 10, pady = 10, anchor = "w", fill = 'both')
tkpack (frame.d,  ipady = 10, ipadx = 10, padx = 10, pady = 10, anchor = 'w', fill = 'both')
